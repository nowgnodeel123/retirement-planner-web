...
| ISA(개인종합자산관리계좌) 토글 | 투자자산 단계에서 "이 투자, ISA 계좌인가요?" 3단 선택(아니요/일반형/서민형·농어민형) — 기존 주식/ETF 입력 전체를 ISA 계좌로 간주(신규) |
| 은퇴 가능 나이 계산       | 입력값 기반으로 백엔드가 역산한 결과를 히어로 카드로 표시       |
| 건강보험 피부양자 경고     | 위험한 경우에만(feasible이고 atRisk일 때) 히어로 바로 아래 경고 배너로 노출(M15) |
| 몬테카를로 성공률          | 별도 카드 없이 소득 구성 차트 카드 하단에 한 줄로 노출 — "90세까지 자산이 버틸 확률은 N%예요"(M16) |
...
| 기능 | 설명 |
|---|---|
| 계좌 목록/생성/편집 | 은행/증권사/거래소, 상세유형(일반/ISA/IRP/연금저축). 헤더의 **"관리" 토글**로 수정·삭제 아이콘 노출(D-182) — 좌측 스와이프(`SwipeableRow`, D-175) 방식은 실기기에서 탭과 자주 혼동돼 계좌 상세 진입이 막히는 문제로 되돌림, 이름 수정 모달(`RenameAccountModal`) |
| 계좌 상세 3탭 | 자산 / 수익 / 세금 탭 구조. 은행 계좌는 수익·세금 탭 자체가 개념이 없어 렌더링하지 않음 |
| 자산 추가 | 계좌 기관유형·상세유형에 따라 카테고리 자동 필터 — 증권사=국내·해외주식, 거래소=코인, 은행=미지원, **연금저축·IRP는 매수 자체를 지원하지 않음**(D-190, ★핵심 — 두 계좌는 지정 상품만 거래 가능해 개별주 매수가 원천적으로 불가능한데 지금까지 아무 제약이 없었음, 백엔드 `AssetService.buy()`와 이중 방어). **증권사 계좌는 국내·해외주식 통합 검색**(`UnifiedStockSearch`, KRX 캐시+Finnhub 동시 조회, D-170), **코인도 동일 패턴의 종목 검색**(`CryptoSearch`, Upbit KRW마켓 목록 5분 캐싱, D-189) — 세 카테고리 모두 이름 검색→선택만으로 등록, 종목코드를 직접 입력하거나 답처럼 보여주지 않음 |
| 종목 코드 표시 | 검색 결과·선택 요약·보유자산 목록·자산 상세·도넛 범례 전체에서 종목명 옆에 티커(symbol)를 회색 보조 텍스트로 표시(D-189) — 선택의 "정답"으로 강조하지 않되 참고 정보는 어디서나 확인 가능 |
...
| 자산 상세 | 보유 요약, 매수/매도 폼(재고 추가 가능), 배당 등록 폼(국내·해외주식만 노출, 배당락일 선택 입력 포함), 매매+배당 통합 히스토리 |
...
| 기능 | 설명 |
|---|---|
| 대시보드(`/portfolio`) | 헤더 좌측에 둥지+알 마크(D-188), 총자산 → **종목별 비중 도넛**(D-136, 계좌를 넘나들며 심볼로 합산) 순서로 배치. 계좌·자산이 하나도 없어도 총자산 0원 + 빈 도넛을 기본 표시(D-183, 이전엔 통째로 숨겨짐). 계좌 카드에 계좌별 평가금액·손익률 표시. "이번 달 매매+배당" 인사이트 배너는 액션 불가능한 정보로 판단해 제거(D-181) |
...
| 기능 | 설명 |
|---|---|
| 박스 + ≡ 메뉴 구조 | 포트폴리오/은퇴시뮬레이션 2탭은 세그먼트 컨트롤 박스(선택 시 슬라이드하는 하이라이트 유지) 하나로 묶고, "내 정보"는 그 옆 별도 ≡ 메뉴 버튼으로 분리(D-182) — 이전에 시도했던 3탭 배지(D-181)·원형 FAB(D-172~D-173) 구조를 모두 대체 |
| ≡ 메뉴 팝업 | 버튼 위로 슬라이드하는 팝업(`NavMenu`), 내 정보(`/my` 이동)/화면 테마(라이트·다크 즉시 전환)/**로그아웃**. 알림 설정은 여기 없음 — MY 탭과 항목이 겹치지 않도록 역할을 나눔(D-184/D-185) |
| 아바타 | 추상 기하 마크 10종(가입 시 서버가 무작위 배정) — 사람 얼굴 아님 |
| `/my` | 프로필 카드(아바타+닉네임+이메일, 탭하면 `/my/profile`), 일반(**알림 설정** 준비중), 지원(기능 제안하기 mailto, 이용약관·개인정보처리방침), 계정(**회원탈퇴** — D-190 세션에서 로그아웃을 ≡ 메뉴로 옮기며 이 자리를 대체) |
| 회원탈퇴 | `WithdrawModal` — LOCAL 계정은 현재 비밀번호 재확인 후 진행, 카카오 계정은 확인 문구만으로 진행. 성공 시 토큰 삭제 후 즉시 로그아웃 |
| `/my/profile` | 닉네임(수정 버튼 클릭 시 인라인 편집)·이름·생년월일·성별·휴대전화(재인증) 수정, **비밀번호 변경**(현재 비밀번호 검증) — 카카오 계정은 개인정보/비밀번호 섹션 숨김(D-177) |
...
│   ├── page.tsx                        # 프로필 카드 + 일반/지원/계정 섹션 (D-172~D-176)
│   └── profile/page.tsx                # 개인정보 수정 화면 — 닉네임/이름/생년월일/성별/휴대전화/비밀번호 (D-177)
├── globals.css                         # 라이트/다크 CSS 변수 (--gain/--loss/--gain-soft/--loss-soft/--accent/--warning/--warning-soft 등)
├── components/
│   ├── auth/
│   │   └── RequireAuth.tsx             # 로그인 필요 화면 공용 가드 (포트폴리오 + 시뮬레이터 + MY 탭 공유)
│   ├── nav/
│   │   ├── BottomTabBar.tsx            # 포트폴리오/은퇴시뮬레이션 세그먼트 박스 + ≡ 메뉴 버튼(NavMenu: 내 정보/화면 테마/로그아웃, D-182/D-185)
│   │   └── ThemeInit.tsx
│   ├── profile/
│   │   ├── Avatar.tsx                  # 추상 기하 마크 10종 렌더링 (avatarId % 10)
│   │   └── WithdrawModal.tsx           # 회원탈퇴 확인 모달 — LOCAL은 비밀번호 재확인, 카카오는 확인만(D-190)
│   ├── wizard/
...
│   │   ├── Step3InvestmentAssets.tsx   # 주식 / ETF + ISA 계좌 유형 토글(신규)
│   │   ├── AnalyzingScreen.tsx         # 제출 시 4단계 분석 연출 화면, 최소 2.2초 노출(D-158)
│   │   ├── ResultScreen.tsx            # 결과 화면 (히어로 + 피부양자 경고 + 차트(+몬테카를로 한 줄) + What-if 슬라이더 + 액션 버튼), count-up 훅 포함
...
│   │   └── types.ts                    # 은퇴 시뮬레이터 DTO 타입 + 변환 함수 (dependentStatusWarning/monteCarloResult/isaType 포함)
│   └── portfolio/
│       ├── types.ts                    # 계좌/자산/거래/배당/수익/세금 DTO 타입 (CryptoSearchResult 포함)
│       ├── format.ts                   # 금액 포맷 등 공용 유틸
│       ├── PortfolioSummary.tsx        # 총자산/손익 요약 — 자산 0건이어도 0원 표시(D-183)
│       ├── HoldingsDonutChart.tsx      # 종목별 비중 도넛(D-136), 종목명 옆 티커 표시(D-189), 빈 상태 플레이스홀더 링(D-183)
│       ├── AccountCard.tsx             # 계좌 목록 개별 행 — manageMode prop으로 평소엔 링크, 관리 모드에선 수정/삭제 아이콘(D-182)
│       ├── CategoryBadge.tsx           # 카테고리별 색상 배지(국내·해외주식/코인)
...
└── portfolio/
    ├── layout.tsx                       # RequireAuth 적용
    ├── page.tsx                         # 포트폴리오 메인(대시보드) — 헤더 둥지 아이콘(D-188), "관리" 토글로 AccountCard manageMode 전환(D-182, 좌측 스와이프 방식 폐기)
    └── accounts/
        ├── new/page.tsx
        └── [accountId]/
            ├── page.tsx                 # 계좌 상세 — 자산/수익/세금 3탭(은행은 수익·세금 탭 미렌더링), 보유자산 목록에 종목명 옆 티커 표시(D-189)
            └── assets/
                ├── new/page.tsx         # 자산 추가 — UnifiedStockSearch(국내+해외, D-170)·CryptoSearch(코인, D-189) 로컬 컴포넌트, TradeAmountFields 재사용, 연금저축·IRP는 진입 시 안내로 대체(D-190)
                └── [assetId]/page.tsx   # 자산 상세: TradeForm(매수/매도 공용) + 배당 등록(배당락일 선택 입력) + 통합 히스토리(매수=빨강/매도=파랑), 제목 옆 티커 표시(D-189)
...
**Step 3 — 투자 자산**: 주식/ETF 월 납입액·기대수익률·현재 잔액 + ISA 계좌 유형 토글(아니요/일반형/서민형·농어민형, 신규)

**제출 → 결과** — 제출 시 `AnalyzingScreen`(데이터 확인→국민연금/퇴직연금 계산→물가상승률 반영→시뮬레이션 실행 4단계, 최소 2.2초)을 거쳐 결과 화면 블록이 순차 등장(D-158)

**결과 화면** — 히어로(count-up 애니메이션) + [건강보험 피부양자 경고 배너, 위험한 경우만] + 차트(+몬테카를로 성공률 한 줄) + What-if 슬라이더 + 액션 버튼(D-124~D-131 반복 단순화 + D-157/D-158/M15/M16)
- 히어로: 예상 은퇴 가능 나이 (달성 시 파란 톤, 75세까지 미달성 시 주황 경고 톤), 숫자는 count-up으로 등장
- 건강보험 피부양자 경고: feasible이고 위험 신호(atRisk)일 때만 호박색 배너로 노출(M15)
- 연도별 소득 구성 영역 차트(국민연금/퇴직연금/IRP+연금저축/주식·ETF 4계열, 오늘 가치 기준) + 목표 생활비 점선 + 은퇴 나이 기준선, 카드 하단에 몬테카를로 성공률 한 줄(M16)
...
**메인**: 헤더 좌측 둥지+알 아이콘(D-188) → 총자산 요약(자산 0건이어도 0원, D-183) → 종목별 비중 도넛(D-136, 빈 상태는 플레이스홀더 링) → 계좌 목록(계좌별 평가금액·손익률 표시, 헤더 "관리" 토글로 수정/삭제 아이콘 전환, D-182)
...
- 배당 등록 폼(국내·해외주식만 노출, 해외는 환율 필수, 배당락일 선택 입력)
- 매매+배당 통합 히스토리 — 매수=빨강/매도=파랑 배지(D-138), 배당은 별도 배지

### 하단 내비게이션 — 포트폴리오/은퇴시뮬레이션 박스 + ≡ 메뉴 (로그인 필요)

포트폴리오·은퇴시뮬레이션 2탭은 세그먼트 박스 하나로 묶이고, "내 정보"는 그 옆 ≡ 메뉴 버튼으로 분리돼 있습니다(D-182). ≡ 메뉴를 누르면 위로 슬라이드하는 팝업이 뜨고, 여기서 **내 정보**(`/my` 이동)·**화면 테마**(라이트/다크 즉시 전환)·**로그아웃**을 바로 처리할 수 있습니다.

### MY (`/my`, 로그인 필요)

프로필 카드(아바타+닉네임+이메일, 탭하면 `/my/profile`) → 일반(**알림 설정** 준비중) → 지원(기능 제안하기 mailto, 이용약관·개인정보처리방침) → 계정(**회원탈퇴**). 로그아웃은 ≡ 메뉴로 옮겨가서 이 화면의 "계정" 섹션은 세션을 끊는 액션이 아니라 계정 자체를 없애는 회원탈퇴로 바뀌었습니다(D-185/D-190).

**회원탈퇴** — 확인 모달(`WithdrawModal`)에서 LOCAL 계정은 현재 비밀번호를 재입력해야 진행되고(오답 시 에러 배너), 카카오 계정은 확인 문구만으로 진행됩니다. 성공하면 계좌·자산·거래 내역이 전부 삭제되고 즉시 로그아웃됩니다 — 되돌릴 수 없습니다.

**`/my/profile`** — 닉네임(수정 버튼으로 인라인 편집), 이름/생년월일/성별/휴대전화(SMS 재인증) 수정, **비밀번호 변경**(현재 비밀번호 검증). 카카오 계정은 개인정보·비밀번호 섹션을 숨기고 안내 문구만 표시(D-177).
...
| 변수 | 용도 |
|---|---|
| `--text-strong` / `--text` / `--text-sub` / `--text-faint` | 텍스트 명도 단계. `--text-faint`는 WCAG 최소 대비(3:1, UI/보조텍스트) 기준으로 실측 조정됨(라이트 3.04:1/다크 3.6:1) |
...
MVP 개발 페이즈 M1~M13이 모두 완료된 뒤, 상용화 목표 전환(D-161) 이후 백엔드 M14~M16에 맞춘 프론트 개선이 이어졌습니다.
...
2026-08-18부로 목표를 "이직 포트폴리오"에서 **실제 상용 서비스 수익화**로 전환(D-161, 백엔드 M14~M16과 같은 흐름) 이후:
- [x] 건강보험 피부양자 자격상실 경고 배너, 몬테카를로 성공률 한 줄 노출(M15/M16)
- [x] 국내/해외주식 통합 검색(`UnifiedStockSearch`) — 자산 등록 시 카테고리 사전 선택 제거(D-170)
- [x] 계좌 목록 전역 "관리" 토글 제거 → 좌측 스와이프 방식(`SwipeableRow`)으로 전환(D-175)
- [x] 프로필 드롭다운 제거 → 하단 탭바 원형 FAB "MY" 탭으로 승격, 아바타 시스템(추상 기하 마크 10종) 도입(D-172~D-173)
- [x] 마이페이지 개인정보 수정 화면(`/my/profile`) 신규 — 이름/생년월일/성별/휴대전화/비밀번호 변경(D-177)
- [x] WCAG 명도대비 실측 조정(`--text-faint`) + 디자인 토큰 우회 하드코딩 정리(D-174)
- [x] 배당 등록 폼에 배당락일(ex-date) 선택 입력 필드 추가
- [x] ISA(개인종합자산관리계좌) 유형 토글 — 위저드 3단계에서 선택

같은 흐름, 사용자 UI 재검토 요청으로 진행한 애드혹 개선:
- [x] 하단 내비 재구성 — 포트폴리오/은퇴시뮬레이션 세그먼트 박스 + ≡ 메뉴(내 정보/화면 테마/로그아웃) 분리(D-182)
- [x] 계좌 목록 좌측 스와이프(`SwipeableRow`, D-175) 폐기 → 헤더 "관리" 토글 방식으로 재전환 — 실기기에서 스와이프가 탭과 혼동돼 계좌 상세 진입이 막히는 문제
- [x] 포트폴리오 메인 빈 상태에서도 총자산 0원 + 빈 도넛 기본 표시(D-183), 헤더에 둥지+알 아이콘(직선 없는 베지어 곡선 기반, D-186~D-188)
- [x] MY 탭 계정 섹션을 로그아웃 → **회원탈퇴**로 교체, 백엔드 `DELETE /api/users/me` 신규(LOCAL 비밀번호 재확인, 카카오는 확인만) — 알림 설정은 MY 탭으로, 로그아웃은 ≡ 메뉴로 역할 분리(D-184/D-185/D-190)
- [x] 종목 코드(티커)를 검색 답으로 보여주지 않고, 선택 후에는 종목명 옆 회색 보조 텍스트로 어디서나 노출(D-189)
- [x] 코인도 국내·해외주식과 동일하게 이름 검색 자동완성(`CryptoSearch`, Upbit KRW마켓) — 수동 종목코드 입력 제거(D-189)
- [x] **연금저축·IRP 계좌 개별 매수 차단**(★핵심, D-190) — 실제로는 지정 상품만 거래 가능한데 이 앱은 지금까지 아무 제약 없이 개별 국내·해외주식 매수를 허용하고 있었음(실제 데이터에서 연금저축 계좌에 매수·매도된 개별주 이력 발견). ETF/펀드 구분 데이터가 없어 "개별주만 차단"은 불가능해 두 계좌 유형 모두 매수 자체를 막기로 확정, 백엔드와 이중 방어
