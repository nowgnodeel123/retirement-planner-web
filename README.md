...
- **포트폴리오** — 계좌(증권사/거래소) 관리, 국내·해외주식/암호화폐/ETF 종목 검색 자동완성, 매수/매도와 거래 정정·삭제, 현금·외화(원화/달러) 잔액 입력, 배당 등록, 계좌·종목 좌측 스와이프로 이름 수정·삭제, 순서 편집 화면에서 원하는 순서로 정렬, 보유자산 대시보드(총자산·종목별 비중 도넛·자산 등급)
