...
| 기능 | 설명 |
|---|---|
| 계좌 목록/생성/편집 | 은행/증권사/거래소, 상세유형(일반/ISA/IRP/연금저축). 헤더의 **"관리" 토글**로 수정·삭제 아이콘 노출(D-193) — 좌측 스와이프(`SwipeableRow`, D-175) 방식은 실기기에서 탭과 자주 혼동돼 계좌 상세 진입이 막히는 문제로 되돌림, 이름 수정 모달(`RenameAccountModal`) |
| 계좌 상세 3탭 | 자산 / 수익 / 세금 탭 구조. 은행 계좌는 수익·세금 탭 자체가 개념이 없어 렌더링하지 않음 |
| 자산 추가 | 계좌 기관유형·상세유형에 따라 카테고리 자동 필터 — 증권사=국내·해외주식, 거래소=코인, 은행=미지원, **연금저축·IRP는 매수 자체를 지원하지 않음**(D-198, ★핵심 — 두 계좌는 지정 상품만 거래 가능해 개별주 매수가 원천적으로 불가능한데 지금까지 아무 제약이 없었음, 백엔드 `AssetService.buy()`와 이중 방어). **증권사 계좌는 국내·해외주식 통합 검색**(`UnifiedStockSearch`, KRX 캐시+Finnhub 동시 조회, D-170), **코인도 동일 패턴의 종목 검색**(`CryptoSearch`, Upbit KRW마켓 목록 5분 캐싱, D-197) — 세 카테고리 모두 이름 검색→선택만으로 등록, 종목코드를 직접 입력하거나 답처럼 보여주지 않음 |
| 종목 코드 표시 | 검색 결과·선택 요약·보유자산 목록·자산 상세·도넛 범례 전체에서 종목명 옆에 티커(symbol)를 회색 보조 텍스트로 표시(D-197) — 선택의 "정답"으로 강조하지 않되 참고 정보는 어디서나 확인 가능 |
...
| 기능 | 설명 |
|---|---|
| 대시보드(`/portfolio`) | 헤더 좌측에 둥지+알 마크(D-194), 총자산 → **종목별 비중 도넛**(D-136, 계좌를 넘나들며 심볼로 합산) 순서로 배치. 계좌·자산이 하나도 없어도 총자산 0원 + 빈 도넛을 기본 표시(D-192, 이전엔 통째로 숨겨짐). 계좌 카드에 계좌별 평가금액·손익률 표시. "이번 달 매매+배당" 인사이트 배너는 액션 불가능한 정보로 판단해 제거(D-183) |
...
| 기능 | 설명 |
|---|---|
| 박스 + ≡ 메뉴 구조 | 포트폴리오/은퇴시뮬레이션 2탭은 세그먼트 컨트롤 박스(선택 시 슬라이드하는 하이라이트 유지) 하나로 묶고, "내 정보"는 그 옆 별도 ≡ 메뉴 버튼으로 분리(D-191) — 이전에 시도했던 다이아몬드 배지 3탭(D-189)·원형 FAB(D-172~D-173) 구조를 모두 대체 |
| ≡ 메뉴 팝업 | 버튼 위로 슬라이드하는 팝업(`NavMenu`), 내 정보(`/my` 이동)/화면 테마(라이트·다크 즉시 전환)/**로그아웃**. 알림 설정은 여기 없음 — MY 탭과 항목이 겹치지 않도록 역할을 나눔(D-195) |
| 아바타 | 추상 기하 마크 10종(가입 시 서버가 무작위 배정) — 사람 얼굴 아님 |
| `/my` | 프로필 카드(아바타+닉네임+이메일, 탭하면 `/my/profile`), 일반(**알림 설정** 준비중), 지원(기능 제안하기 mailto, 이용약관·개인정보처리방침), 계정(**회원탈퇴** — D-196 세션에서 로그아웃을 ≡ 메뉴로 옮기며 이 자리를 대체) |
...
│   ├── nav/
│   │   ├── BottomTabBar.tsx            # 포트폴리오/은퇴시뮬레이션 세그먼트 박스 + ≡ 메뉴 버튼(NavMenu: 내 정보/화면 테마/로그아웃, D-191/D-195)
│   │   └── ThemeInit.tsx
│   ├── profile/
│   │   ├── Avatar.tsx                  # 추상 기하 마크 10종 렌더링 (avatarId % 10)
│   │   └── WithdrawModal.tsx           # 회원탈퇴 확인 모달 — LOCAL은 비밀번호 재확인, 카카오는 확인만(D-196)
│   ├── wizard/
...
│       ├── PortfolioSummary.tsx        # 총자산/손익 요약 — 자산 0건이어도 0원 표시(D-192)
│       ├── HoldingsDonutChart.tsx      # 종목별 비중 도넛(D-136), 종목명 옆 티커 표시(D-197), 빈 상태 플레이스홀더 링(D-192)
│       ├── AccountCard.tsx             # 계좌 목록 개별 행 — manageMode prop으로 평소엔 링크, 관리 모드에선 수정/삭제 아이콘(D-193)
...
└── portfolio/
    ├── layout.tsx                       # RequireAuth 적용
    ├── page.tsx                         # 포트폴리오 메인(대시보드) — 헤더 둥지 아이콘(D-194), "관리" 토글로 AccountCard manageMode 전환(D-193, 좌측 스와이프 방식 폐기)
    └── accounts/
        ├── new/page.tsx
        └── [accountId]/
            ├── page.tsx                 # 계좌 상세 — 자산/수익/세금 3탭(은행은 수익·세금 탭 미렌더링), 보유자산 목록에 종목명 옆 티커 표시(D-197)
            └── assets/
                ├── new/page.tsx         # 자산 추가 — UnifiedStockSearch(국내+해외, D-170)·CryptoSearch(코인, D-197) 로컬 컴포넌트, TradeAmountFields 재사용, 연금저축·IRP는 진입 시 안내로 대체(D-198)
                └── [assetId]/page.tsx   # 자산 상세: TradeForm(매수/매도 공용) + 배당 등록(배당락일 선택 입력) + 통합 히스토리(매수=빨강/매도=파랑), 제목 옆 티커 표시(D-197)
...
**메인**: 헤더 좌측 둥지+알 아이콘(D-194) → 총자산 요약(자산 0건이어도 0원, D-192) → 종목별 비중 도넛(D-136, 빈 상태는 플레이스홀더 링) → 계좌 목록(계좌별 평가금액·손익률 표시, 헤더 "관리" 토글로 수정/삭제 아이콘 전환, D-193)
...
포트폴리오·은퇴시뮬레이션 2탭은 세그먼트 박스 하나로 묶이고, "내 정보"는 그 옆 ≡ 메뉴 버튼으로 분리돼 있습니다(D-191). ≡ 메뉴를 누르면 위로 슬라이드하는 팝업이 뜨고, 여기서 **내 정보**(`/my` 이동)·**화면 테마**(라이트/다크 즉시 전환)·**로그아웃**을 바로 처리할 수 있습니다.

### MY (`/my`, 로그인 필요)

프로필 카드(아바타+닉네임+이메일, 탭하면 `/my/profile`) → 일반(**알림 설정** 준비중) → 지원(기능 제안하기 mailto, 이용약관·개인정보처리방침) → 계정(**회원탈퇴**). 로그아웃은 ≡ 메뉴로 옮겨가서 이 화면의 "계정" 섹션은 세션을 끊는 액션이 아니라 계정 자체를 없애는 회원탈퇴로 바뀌었습니다(D-195/D-196).
...
- [x] 하단 내비 재구성 — 포트폴리오/은퇴시뮬레이션 세그먼트 박스 + ≡ 메뉴(내 정보/화면 테마/로그아웃) 분리(D-191)
- [x] 계좌 목록 좌측 스와이프(`SwipeableRow`, D-175) 폐기 → 헤더 "관리" 토글 방식으로 재전환(D-193) — 실기기에서 스와이프가 탭과 혼동돼 계좌 상세 진입이 막히는 문제
- [x] 포트폴리오 메인 빈 상태에서도 총자산 0원 + 빈 도넛 기본 표시(D-192), 헤더에 둥지+알 아이콘(직선 없는 베지어 곡선 기반, D-194)
- [x] MY 탭 계정 섹션을 로그아웃 → **회원탈퇴**로 교체, 백엔드 `DELETE /api/users/me` 신규(LOCAL 비밀번호 재확인, 카카오는 확인만, D-196) — 알림 설정은 MY 탭으로, 로그아웃은 ≡ 메뉴로 역할 분리(D-195)
- [x] 종목 코드(티커)를 검색 답으로 보여주지 않고, 선택 후에는 종목명 옆 회색 보조 텍스트로 어디서나 노출(D-197)
- [x] 코인도 국내·해외주식과 동일하게 이름 검색 자동완성(`CryptoSearch`, Upbit KRW마켓) — 수동 종목코드 입력 제거(D-197)
- [x] **연금저축·IRP 계좌 개별 매수 차단**(★핵심, D-198) — 실제로는 지정 상품만 거래 가능한데 이 앱은 지금까지 아무 제약 없이 개별 국내·해외주식 매수를 허용하고 있었음(실제 데이터에서 연금저축 계좌에 매수·매도된 개별주 이력 발견). ETF/펀드 구분 데이터가 없어 "개별주만 차단"은 불가능해 두 계좌 유형 모두 매수 자체를 막기로 확정, 백엔드와 이중 방어
