...
| 기능 | 설명 |
|---|---|
| 계좌 목록/생성/편집 | 은행/증권사/거래소, 상세유형(일반/ISA/IRP/연금저축). **좌측 스와이프로 수정·삭제 액션 노출**(`SwipeableRow`, iOS 메일/설정 앱 스타일, D-175 — 기존 전역 "관리" 토글 제거), 이름 수정 모달(`RenameAccountModal`) |
| 계좌 상세 3탭 | 자산 / 수익 / 세금 탭 구조. 은행 계좌는 수익·세금 탭 자체가 개념이 없어 렌더링하지 않음 |
| 자산 추가 | 계좌 기관유형에 따라 카테고리 자동 필터(증권사=국내·해외주식, 거래소=코인, 은행=미지원). **증권사 계좌는 국내·해외주식 통합 검색**(`UnifiedStockSearch`, KRX 캐시+Finnhub 동시 조회, 한글 포함 여부로 정렬, 선택 즉시 카테고리 자동 확정, D-170) — 코인만 수동입력 |
...
| 기능 | 설명 |
|---|---|
| 하단 탭바 진입점 | 포트폴리오/은퇴시뮬레이션 사이, 탭바 상단 경계 위로 튀어나오는 원형 FAB 버튼(`/my`, D-172~D-173) |
| 아바타 | 추상 기하 마크 10종(가입 시 서버가 무작위 배정) — 사람 얼굴 아님 |
| `/my` | 프로필 카드(아바타+닉네임+이메일, 탭하면 `/my/profile`), 일반(테마 토글, 알림설정 준비중), 지원(기능 제안하기 mailto, 이용약관·개인정보처리방침), 계정(로그아웃) |
| `/my/profile` | 닉네임·이름·생년월일·성별·휴대전화(재인증) 수정, **비밀번호 변경**(현재 비밀번호 검증) — 카카오 계정은 개인정보/비밀번호 섹션 숨김(D-177) |
...
│   ├── nav/
│   │   ├── BottomTabBar.tsx            # 포트폴리오 / 원형 FAB("내 정보", /my) / 은퇴시뮬레이션 3그리드
│   │   └── ThemeInit.tsx
│   ├── profile/
...
│   └── portfolio/
│       ├── types.ts                    # 계좌/자산/거래/배당/수익/세금 DTO 타입 (exDividendDate 포함)
...
│       ├── SwipeableRow.tsx            # 좌측 스와이프로 수정·삭제 노출, "한 번에 하나만 열림" 공유 레지스트리(D-175)
...
└── portfolio/
    ├── layout.tsx                       # RequireAuth 적용
    ├── page.tsx                         # 포트폴리오 메인(대시보드), AccountCard가 SwipeableRow로 감싸짐
    └── accounts/
        ├── new/page.tsx
        └── [accountId]/
            ├── page.tsx                 # 계좌 상세 — 자산/수익/세금 3탭(은행은 수익·세금 탭 미렌더링)
            └── assets/
                ├── new/page.tsx         # 자산 추가 — UnifiedStockSearch(국내+해외 통합 검색, D-170) 로컬 컴포넌트, TradeAmountFields 재사용
                └── [assetId]/page.tsx   # 자산 상세: TradeForm(매수/매도 공용) + 배당 등록(배당락일 선택 입력) + 통합 히스토리(매수=빨강/매도=파랑)
...
**메인**: 총자산 요약 → 이번 달 매매+배당 인사이트 배너 → 종목별 비중 도넛(D-136) → 계좌 목록(계좌별 평가금액·손익률 표시, 좌측 스와이프로 이름수정/삭제 노출, D-175)
...
하단 탭바 가운데 원형 FAB 버튼으로 진입(D-172~D-173). 프로필 카드(아바타+닉네임+이메일, 탭하면 `/my/profile`) → 일반(테마 토글, 알림설정 준비중) → 지원(기능 제안하기 mailto, 이용약관·개인정보처리방침) → 계정(로그아웃).

**`/my/profile`** — 닉네임 인라인 수정, 이름/생년월일/성별/휴대전화(SMS 재인증) 수정, **비밀번호 변경**(현재 비밀번호 검증). 카카오 계정은 개인정보·비밀번호 섹션을 숨기고 안내 문구만 표시(D-177).
